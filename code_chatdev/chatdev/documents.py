...
class Documents():
    ...
    def _rewrite_docs(self):
        ...
        if not os.path.exists(directory):
            os.mkdir(directory)
            if print_log: print("{} Created.".format(directory))
        for filename in self.docbooks.keys():
            with open(os.path.join(directory, filename), "w", encoding="utf-8") as writer:
                ...
                if print_log: print(os.path.join(directory, filename), "Writen")
