...
def get_consistency(directory):
    ...
    try:
        ...
        task_code_alignment = 0.0

    return task_code_alignment

def main(warehouse_root, config):
    ...
    tsv_file = "./logs/" + config + "_eval_quality.tsv"
    ...
    with open(tsv_file, "a", encoding="utf-8") as writer:
        for i, directory in enumerate(directories):
            ...
            write_string(f"{directory}, Completeness: {completeness:.2f}, Execution: {executability:.2f}, Consistency: {consistency:.2f}\n")

            if consistency == 0.0:
                print(f"   {i+1} unsucessfully get the consistency score. ")
                unsuccess_count += 1
                continue
            ...
            counter += 1
    print('Mean completeness:', np.mean(completeness_list))
    print('Mean executability:', np.mean(executability_list))
    print('Mean consistency:', np.mean(consistency_list))
    print("unsuccess count:", unsuccess_count)
    results_dict = {
        'mean_completeness': np.mean(completeness_list),
        'mean_executability': np.mean(executability_list),
        'mean_consistency': np.mean(consistency_list),
        'unsuccess_count': unsuccess_count,
    }
    result_file = './logs/' + config + '_results.json'
    with open(result_file, "w", encoding="utf-8") as writer:
        writer.write(json.dumps(results_dict, indent=2))
    return results_dict
...
if __name__ == '__main__':
    ...
    config = args.config
    main(warehouse_root = "./WareHouse", config=config)
