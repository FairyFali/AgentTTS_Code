...
class ChatChain:
    ...
    def __init__(self,
        ...
        # there are lots of config information in these files.
        ...
        # there are a lot of function var inside chat_env.
        ...
        for role in self.config_role:  # each role have a corresponding prompt template. the roles include CTO, CEO, CPO, Programmer, reviewer, tester, Counselor, HRO
            self.role_prompts[role] = "\n".join(self.config_role[role])

        # init log file and start time.
        ...
        self.phases = dict()  # build a phase dict {'phase name': phase instance}
        for phase in self.config_phase:
            assistant_role_name = self.config_phase[phase]['assistant_role_name']  # role name of assistant
            user_role_name = self.config_phase[phase]['user_role_name']  # role name of chief
            phase_prompt = "\n\n".join(self.config_phase[phase]['phase_prompt'])  # current prompt
            phase_class = getattr(self.phase_module, phase)
            repeating = self.config_phase[phase].get('repeating', 1)
            if "model" in self.config_phase[phase]:
                model = self.config_phase[phase]['model']
            ...
            print('phase type: ', phase, model_type)
            # init an instance of Phase.
            # 我可以修改这里让每个阶段使用不同的model
            # 我也可以修改这里加入每个阶段的采样次数
            #
    ...
    def execute_step(self, phase_item: dict):
        ...
        if phase_type == "SimplePhase":
            ...
            if phase in self.phases:
                print('Log, phase name:', self.phases[phase].phase_name, self.phases[phase].user_role_name, self.phases[phase].assistant_role_name, self.phases[phase].model_type)
        ...
        # For ComposedPhase, we create instance here then conduct the "ComposedPhase.execute" method
            ...
            print("Log, composed phase:", phase, composition)
    ...
    def get_logfilepath(self):
        ...
        directory = os.path.join(root, self.warehouse)
    ...
    def pre_processing(self):
        ...
        if self.chat_env.config.clear_structure:  # delete the files except .py and .log files
        ...
        chat_gpt_config = ChatGPTConfig()  # config for chat gpt
        ...
        if print_log: log_visualize(preprocess_msg)
    ...
    def post_processing(self):
        ...
        if self.chat_env_config.git_management:
            ...
            log_git_info += "cd {}; git commit -m \"v{} Final Version\"\n".format(self.chat_env.env_dict["directory"],
                                                                                  self.chat_env.codes.version)
            if print_log: log_visualize(log_git_info)
            ...
            git_info += log_output
            if print_log: log_visualize(git_info)
        ...
        if print_log: log_visualize(post_info)

        logging.shutdown()
        time.sleep(1)

        shutil.move(self.log_filepath,
                    os.path.join(root + "/" + self.warehouse, "_".join([self.project_name, self.org_name, self.start_time]),
                                 os.path.basename(self.log_filepath)))
        if os.path.exists(self.log_filepath):
            os.unlink(self.log_filepath)
...
then you should return a message in a format like \"<INFO> revised_version_of_the_description\", do not return messages in other formats.""".format(
        ...
        if print_log: log_visualize(role_play_session.assistant_agent.role_name, assistant_response.msg.content)
        if print_log:
            log_visualize(
